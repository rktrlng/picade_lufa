typedef struct
{
  ...
  int  port;       // which port is this input on? 0 = PORTB, 1 = PORTF, 2 = PORTD, 3 = PORTC
...
input inputs[] = {
  ...
  /* GPIO 345 on Port B-456, GPIO 6 on Port C-6 */
...
void setup() {
  /*
    Setup the ports to be inputs with pullup resistors enabled
  */
  DDRB = DDRD = DDRF = DDRC = B00000000; // set ports B, D, F, C to be inputs
...
void loop() {
  ...
  */
  ...
  char pinStates[4] = {PINB, PINF, PIND, PINC}; // read the current port states and store in 0, 1, 2, 3 indexed array to match our port ids above
